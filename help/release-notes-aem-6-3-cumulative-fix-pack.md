...
description: AEM 6.3 - Note sulla versione Cumulative Fix Pack
...
# AEM 6.3 Cumulative Fix Pack - Note sulla versione {#release-notes-aem-cumulative-fix-pack}
...
|---|---|
| **Versione** | 6.3 |
| **Versione** | Cumulative Fix Pack 6.3.3.8 su [PackageShare](https://www.adobeaemcloud.com/content/marketplace/marketplaceProxy.html?packagePath=/content/companies/public/adobe/packages/cq630/cumulativefixpack/AEM-CFP-6.3.3.8), [Distribuzione software(Beta)](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq630/cumulativefixpack/aem-6.3.3-cfp-8.0.zip) |
| **Prerequisito** | [AEM 6.3 Service Pack 3 (6.3.3.0)](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html) |
| **Disponibilità generale** | 05 marzo 2020 |
...
Per ulteriori informazioni su CFP e altri tipi di rilasci, vedere [Definizioni dei veicoli con rilasci di manutenzione.](https://docs.adobe.com/content/docs/en/aem/6-3/deploy/maintenance-release-vehicle-definitions.html)
...
AEM Cumulative Fix Pack 6.3.3.8 dipende da AEM 6.3 Service Pack 3. È pertanto necessario installare il pacchetto AEM Cumulative Fix Pack 6.3.3.x dopo aver installato AEM 6.3 Service Pack 3. Per le istruzioni di installazione, consulta le [note sulla versione di AEM 6.3 Service Pack 3](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html).
...
>[!NOTE]
>
>Per AEM istanze con una versione inferiore a 6.3.3.0,  Adobe consiglia di distribuire SP/CFP tramite la cartella di installazione per i clienti che hanno un numero elevato di utenti AEM&#39;istanza.
...
* La pagina Aggiungi alla raccolta non mostra tutte le raccolte disponibili nella vista a schede quando si aggiungono risorse alla raccolta (NPR-32537).

### Sites {#sites}

* Quando selezionate un parsys e un componente al suo interno e utilizzate la scelta rapida da tastiera per eliminare gli elementi selezionati, l&#39;azione elimina sia il componente che i relativi parsys padre (NPR-32071).
...
* ReportSuitesServlet è vulnerabile all&#39;SSRF (NPR-32162).

### Targeting delle campagne {#campaign-targeting}

* Il contenuto di un componente modificato, nell’istanza Author e quindi attivato, non è visibile nell’istanza Pubblica finché il componente non viene riavviato **com.day.cq.personalization.impl.TargetingContentManagerImpl** (NPR-32489 e NPR-32232).
* Le prestazioni di Contexthub si bloccano in seguito alla pubblicazione (NPR-31170).
...
>[!NOTE]
>
>I pacchetti di componenti aggiuntivi per AEM Forms consentono di allineare le funzionalità di Forms ai Service Pack e i Cumulative Fix Pack di AEM. Pertanto, è necessario installare  pacchetto aggiuntivo AEM Forms dopo l&#39;installazione di AEM Service Pack, Cumulative Fix Pack o Feature Pack.

* Designer: Se l&#39;opzione relativa ai tag è abilitata, il bordo del sottomodulo scompare nell&#39;output PDF generato (NPR-32324 e NPR-32545).
* Designer: Se in una tabella sono presenti celle unite, il test di accessibilità non riesce per il file PDF di output convertito da un modulo XDP utilizzando il servizio di output (NPR-32068).
...
AEM Cumulative Fix Pack 6.3.3.7 dipende da AEM 6.3 Service Pack 3. È pertanto necessario installare il pacchetto AEM Cumulative Fix Pack 6.3.3.x dopo aver installato AEM 6.3 Service Pack 3. Per le istruzioni di installazione, consulta le [note sulla versione di AEM 6.3 Service Pack 3](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html).
...
* La variabile `${extension}` non viene rappresentata nell&#39;istanza di creazione nell&#39;elaborazione del flusso di lavoro (NPR-31694).
* Il valore di scadenza (durata della cache cliente) configurato per la modalità ibrida di Dynamic Media non viene replicato nell’ambiente di distribuzione di Dynamic Media (NPR-31114).
* Le risorse vengono replicate nell’istanza di pubblicazione dall’istanza di creazione in esecuzione nella distribuzione di Dynamic Media - Scene7 anche dopo l’utilizzo di filtri predefiniti (NPR-30856).
...
* Le configurazioni di rollout non vengono recuperate correttamente da blueprintConfig sul nodo principale. Viene attivata la disattivazione attivata sia per le blueprint che per le Live Copy. La disattivazione deve essere attivata solo per la blueprint (NPR-30866).
...
* Quando un utente sposta lo stato attivo sui campi della barra a sinistra e utilizza una scelta rapida da tastiera per incollare il contenuto, viene incollato il contenuto degli Appunti dell’Editor pagina invece del contenuto copiato dai campi della barra a sinistra (NPR-31173).
...
* I valori a virgola mobile calcolati utilizzando uno script non vengono visualizzati correttamente in un modulo che fa parte di un set modulo (NPR-30802).
...
AEM Cumulative Fix Pack 6.3.3.6 dipende da AEM 6.3 Service Pack 3. È pertanto necessario installare il pacchetto AEM Cumulative Fix Pack 6.3.3.x dopo aver installato AEM 6.3 Service Pack 3. Per le istruzioni di installazione, consulta le [note sulla versione di AEM 6.3 Service Pack 3](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html).
...
#### Forms - Integrazione con il back-end {#forms-backend-integration}
...
AEM Cumulative Fix Pack 6.3.3.5 dipende da AEM 6.3 Service Pack 3. È pertanto necessario installare il pacchetto AEM Cumulative Fix Pack 6.3.3.x dopo aver installato AEM 6.3 Service Pack 3. Per le istruzioni di installazione, consulta le [note sulla versione di AEM 6.3 Service Pack 3](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html).
...
* Aggiornamento dell’interfaccia DMGGateway di DAM per il supporto multipart di S3. NPR-29740: Hotfix per Q-4226303
...
* (Frammenti esperienza) FacebookApplication#setUpApp utilizza API obsolete, che non funziona più. NPR-29213: Hotfix per CQ-4266630
...
* I dati forniti dall&#39;utente non sono preceduti da un carattere di escape nell&#39;output del componente `cq/replication/components/agent`e ciò comporta una vulnerabilità XSS (Cross-Site Scripting) memorizzata. NPR-29452: Hotfix per CQ-4266263
...
AEM Cumulative Fix Pack 6.3.3.4 dipende da AEM 6.3 Service Pack 3. È pertanto necessario installare il pacchetto AEM Cumulative Fix Pack 6.3.3.x dopo aver installato AEM 6.3 Service Pack 3. Per le istruzioni di installazione, consulta le [note sulla versione di AEM 6.3 Service Pack 3](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html).
...
* Se si ricarica un archivio con lo stesso nome, i rendering per le nuove risorse elaborate non vengono generati. NPR-28643: Hotfix per CQ-4262286
...
* Per la distribuzione di video desktop DMS7 con DMComponent si utilizza il download progressivo e non lo streaming per la riproduzione di video in modalità di pubblicazione. NPR-28754: Hotfix per CQ-4263732
...
* (MSM) Implementa LiveCopyIndex utilizzando l&#39;indice di rovere. NPR-29198: Hotfix per CQ-4222472
* Il file coral.js include una versione vulnerabile della libreria handlebars.js. NPR-26973: Hotfix per CQ-4255377
* L&#39;utilizzo di un componente Target con un Contenitore di layout e un Componente testo nidificati genera un errore JavaScript &quot;Impossibile leggere la proprietà currentPos di null&quot; quando si modifica il testo o si fa clic sul contenitore. NPR-29077: Hotfix per CQ-4246594
* (Interfaccia utente touch) Non è possibile aggiornare in blocco i tag alle pagine già etichettate da diversi tag. NPR-28729: Hotfix per CQ-4262922
...
* (Configurazioni Cloud Service) La casella di controllo &quot;ereditato da&quot; visualizzata al livello principale deve essere rimossa. NPR-28771: Hotfix per CQ-4259676
...
* Facendo clic sul tag si rimuove l’evento globale di rilascio del mouse e la finestra di dialogo rimane bloccata nella “modalità trascinabile”. NPR-28641: Hotfix per CUI-7294

### WCM - MSM {#wcm-msm}

* Il rollout pushOnModify per lo spostamento PageManager si impegna più volte da due sessioni causando una condizione di gara. NPR-28880: Hotfix per CQ-4266191
...
* Abilitazione dell’opzione per la selezione di elementi per pagina nella pagina per la visualizzazione dei set di criteri. NPR-29243: Hotfix per CQ-4268567 e CQ-4265132
...
### Programma di installazione di JEE per Forms  {#forms-jee-installer-3}
...
AEM Cumulative Fix Pack 6.3.3.3 dipende da AEM 6.3 Service Pack 3. È pertanto necessario installare il pacchetto AEM Cumulative Fix Pack 6.3.3.x dopo aver installato AEM 6.3 Service Pack 3. Per le istruzioni di installazione, consulta le [note sulla versione di AEM 6.3 Service Pack 3](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html).

Gli elementi di rilievo di **AEM Cumulative Fix Pack** sono:

* Aggiornamento dell’archivio incorporato (Apache Jackrabbit Oak) alla versione 1.6.16.
* La paginazione dell’elenco di impostazioni criteri viene limitata a 50 record per pagina.
* Aggiunto rappresentante: nella cache nei nodi ignorabili in  listener di sincronizzazione utenti AEM Communities sulle istanze di pubblicazione.
* È stata aggiunta l&#39;etichetta aria per il pulsante di visualizzazione elenco e a schede.
...
* Le impostazioni di pubblicazione di ImageServer non vengono replicate in modalità ibrida. NPR-28329: Hotfix per CQ-4253030
...
* Attivazione della navigazione tramite tastiera per attivare la riproduzione del contenuto. NPR-27738: Hotfix per CQ-4254026
* È stata aggiunta l&#39;etichetta aria per il pulsante di visualizzazione elenco e a schede. NPR-27736: Hotfix per CQ-4254027
* (Backport) Aggiunto rep: nella cache nei nodi ignorabili in  listener di sincronizzazione utenti AEM Communities sulle istanze di pubblicazione. NPR-27841: Hotfix per CQ-4247234
* I caratteri speciali hanno il prefisso escape (\) nella casella di ricerca a livello di interfaccia utente. NPR-27839: Hotfix per CQ-4259757
* Errore durante la ricerca di caratteri come ( , +,? nella ricerca rapida. NPR-28212: Hotfix per CQ-4260969
* Impossibile utilizzare l’API per eliminare i commenti in UGC (User-Generated Content). NPR-28075: Hotfix per CQ-4260534
...
* Se si esegue Version Purge in AEM 6.3, nei registri viene visualizzato un avviso ripetuto. NPR-27750: Hotfix per CQ-4206870
...
* L&#39;elenco &#39;loaderPromises&#39; non viene cancellato dopo il caricamento della cornice contenuto in editor.js NPR-27768: Hotfix per CQ-4205337
* Impossibile impostare i criteri dei modelli in parsys nidificati senza impostare il componente padre. NPR-27987: Hotfix per CQ-4246095
* Il browser componenti non bonifica l’input dell’utente, pertanto può generare errori JavaScript. NPR-27986: Hotfix per CQ-4247590
* La pagina viene visualizzata vuota quando l&#39;utente tenta di modificare il frammento di contenuto. NPR-27669
...
* Abilitare il supporto per il servizio MS Translator in AEM 6.3 dopo l&#39;aggiornamento di MS Translator a API v3.0. NPR-28365: Hotfix per CQ-4259096
...
* Viene generata un&#39;eccezione con il nome java.lang.IllegalArgumentException message:No enum costante com.adobe.internal.pdfm.docbuilder.signature.PathValidationFailedReason.SIGNED_IN_FUTURE che impedisce il completamento del processo di breve durata. NPR-28652
...
### Forms - Programma di installazione di JEE  {#forms-jee-installer-4}
...
AEM Cumulative Fix Pack 6.3.3.2 dipende da AEM 6.3 Service Pack 3. È pertanto necessario installare il pacchetto AEM Cumulative Fix Pack 6.3.3.x dopo aver installato AEM 6.3 Service Pack 3. Per le istruzioni di installazione, consulta le note sulla versione di AEM 6.3 Service Pack 3.
...
* La sospensione della pagina padre rimuove cq : Tipo di mixin LiveRelationship dalla pagina mancante. NPR-26996: Hotfix per CQ-4254113
* (Controllo collegamenti esterni) I collegamenti interni sono visualizzati come interrotti su singole pagine, ma lo stesso non funziona per i collegamenti esterni. NPR-27481: Hotfix per CQ-4257780
* L’ereditarietà di Configurazioni servizio cloud si interrompe quando si modificano altre proprietà della pagina. NPR-27311: Hotfix per CQ-4256785
* Eccezione Null Pointer quando si utilizza un   insieme a un modulo Foundation. NPR-27333: Hotfix per CQ-4249176
* L’Editor Rich Text rimuove il tag alt vuoto. NPR-26938: Hotfix per CQ-4253267
* (interfaccia classica) Problemi di prestazioni con   Seltionchanged   in caso di più elenchi a discesa. NPR-27115: Hotfix per CQ-4237215
...
* Impossibile creare una versione per Frammenti esperienza. NPR-27689: Hotfix per CQ-4259009
...
* Le azioni    cq : le azioni non vengono considerate per un componente con targeting. NPR-27616: Hotfix per CQ-4257497
...
* Interruzione dell’interfaccia utente di Schema metadati cartelle dopo l’installazione di SP3. Hotfix per CQ-4257737
...
* L’etichetta dello stato di completamento Risorsa Scorm di attivazione risulta vuota nell’interfaccia utente. NPR-27152: Hotfix per CQ-4249994
...
* L&#39;anteprima della traduzione non funziona con il contenuto di esempio .retail. NPR-27170: Hotfix per CQ-4241179

* Correzioni proattive per platform.login. NPR-26961
* Quando si sceglie Salva e chiudi nelle proprietà della pagina, non viene visualizzata la pagina   pagina corretta in AEM WAR con Tomcat. NPR-27567: Hotfix per GRANITE-23671

* Il testo immesso viene perso tramite la funzione sourceEdit una volta salvata. Hotfix per CQ-4259273
...
AEM Cumulative Fix Pack 6.3.3.1 dipende da AEM 6.3 Service Pack 3. È pertanto necessario installare il pacchetto AEM Cumulative Fix Pack 6.3.3.x dopo aver installato AEM 6.3 Service Pack 3. Per le istruzioni di installazione, consulta le [note sulla versione di AEM 6.3 Service Pack 3](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp3-release-notes.html).
...
* Disattiva &quot;Percorso cartella principale società&quot; nell’interfaccia utente di configurazione di Dynamic Media Cloud. NPR-26361: Hotfix per CQ-4249505
...
* (DM Hybrid) La replica del catalogo si interrompe per AEM 6.3.2 con Dynamic Media. NPR-26406: Hotfix per CQ-4251306
...
* (Interfaccia utente touch) La barra degli strumenti non è più utilizzabile con più editor Rich Text quando si passa da una finestra di dialogo a schermo intero a una mobile. NPR-25652: Hotfix per CQ-4206008
* Quando si promuove un lancio con più pagine, vengono create più versioni di ogni pagina. NPR-26810: Hotfix per CQ-4254663
* Le operazioni di spostamento dei tag non vengono rilevate   campi tag modello di frammento di contenuto strutturato. NPR-26801: Hotfix per CQ-4251805
* (Interfaccia touch) L&#39;annullamento della pubblicazione della pagina figlia dall&#39;editor pagina non funziona dopo aver rinominato la pagina nel blueprint. NPR-26774: Hotfix per CQ-4254175
* La pagina di cui è stata annullata la pubblicazione   non funziona con i riferimenti. NPR-26749: Hotfix per CQ-4254372
* La creazione di una variante come Live Copy richiede che l&#39;utente aggiorni la pagina per riflettere. NPR-26663: Hotfix per CQ-4254328
* (Interfaccia classica) Tornando alle proprietà della pagina, l&#39;immagine in miniatura non utilizza più l&#39;ereditarietà e scompare dall&#39;amministratore del sito e dalla barra laterale e l&#39;immagine viene visualizzata come vuota. NPR-26562: Hotfix per CQ-4252346
* Quando si creata una versione di una pagina e si attiva un confronto, i nodi di /content/ La cronologia delle versioni è elencata nell&#39;elenco delle copie live per la Blueprint. NPR-26506: Hotfix per CQ-4243957
* Gli URL nell’editor di amministrazione di Frammenti esperienza non consentono sovrapposizioni. NPR-26318: Hotfix per CQ-4252156
...
* Il trasporto HTTP può causare l’interruzione di sessioni aperte. Hotfix per GRANITE-17434
...
* Modifica del valore dell’elenco a discesa da Dallas a Virginia in Provider risorsa di archiviazione Adobe. NPR-26936: Hotfix per CQ-4254434
* Abilitazione del supporto per la ricerca di titoli localizzati in SocialTagManager. NPR-26932: Hotfix per CQ-4250276
* La miniatura delle immagini di allegati non viene visualizzata quando si crea un post nel forum. NPR-26380: Hotfix per CQ-4253105
* Impossibile caricare più immagini con il plugin Incolla da Word. NPR-26728: Hotfix per CQ-4253638
* Impossibile filtrare il contenuto nella pagina di moderazione. NPR-26697: Hotfix per CQ-4213766
* (Vulnerabilità di sicurezza) Acquisizione dell&#39;account a causa di una configurazione JWT errata. NPR-26440: Hotfix per CQ-4253314
* Il recupero dei dati da Provider risorsa di archiviazione Adobe è lento. NPR-26237: Hotfix per NPR-24152
...
* (Cerca nel forum) Il pulsante Indietro consente di saltare la pagina e di tornare a forum.html. NPR-26949: Hotfix per CQ-4254804
* Il limite massimo per la notifica del conteggio degli elementi non letti è 21. NPR-26947: Hotfix per CQ-4251251
* (processo SearchScheduledPosts) Aggiungere l&#39;interruttore di attivazione/disattivazione in ConfigMgr. NPR-26924: Hotfix per CQ-4250463
...
* (Cerca nel forum) - I collegamenti impaginati ai thread con molti post non funzionano sulle notifiche. Hotfix per CQ-4254202
...
* Eccezione Puntatore Null durante l&#39;aggiunta di gruppi e membri alle impostazioni Ruolo e l&#39;abilitazione del membro privilegiato nella funzione blog. NPR-21732, NPR-27176: Hotfix per CQ-4255909
...
* (multicampo) Consentire agli elementi compositi di avere nomi personalizzati. NPR-26493: Hotfix per GRANITE-20568
...
* (Corallo. Multifield) I campi nidificati utilizzano il modello errato per creare gli elementi. NPR-25649: Hotfix per CUI-6743
* Aggiornate i clientlibs granite coralui2 e coralui3 per rimuovere Handlebars dalla build. NPR-25606: Hotfix per GRANITE-22116
...
* La convalida PDF/A non viene visualizzata come valida con “Validate PDF/A” (Convalida PDF/A). NPR-25934: Hotfix per CQ-4248558

#### Forms - Comunicazione interattiva  {#forms-interactive-communication}

* Le anteprime PDF e HTML della lettera sono visibili e funzionali nella stessa finestra quando si modifica e si salva un modulo modificabile e quindi si apre/chiude l’anteprima PDF. NPR-26770: Hotfix per CQ-4253217
...
* L’impostazione di un modello di ricerca dall’interfaccia utente di amministrazione e quindi lo stesso utilizzo per la ricerca nell’area di lavoro  AEM Forms genera un’eccezione. Hotfix per CQ-4255649
...
* Impossibile caricare esapi.properties come risorsa del caricatore di classe in WAS9. Hotfix per CQ-4255631
...
* La distribuzione di adobe.livecycle.weblogic.ear e adobe.livecycle.websfera.ear non riesce durante l&#39;esecuzione di LCM. Hotfix per CQ-4255629, CQ-4255630
* Messaggi di errore non validi in Gestione applicazione. NPR-23289: Hotfix per CQ-4233163, CQ-4255636
* Quando si fa clic su Add Authentication (Aggiungi autenticazione) durante la configurazione del dominio, viene generato un errore. Hotfix per CQ-4255634
...
AEM Cumulative Fix Pack 6.3.2.2 dipende da AEM 6.3 Service Pack 2. È pertanto necessario installare il pacchetto AEM Cumulative Fix Pack 6.3.2.x dopo aver installato AEM 6.3 Service Pack 2. Per le istruzioni di installazione, consulta le [note sulla versione di AEM 6.3 Service Pack 2](https://helpx.adobe.com/experience-manager/6-3/release-notes/sp2-release-notes.html).
...
* Modifica della lunghezza del tipo di campo in 255 per supportare tutti i tipi MIME possibili per i diversi tipi di allegati.
...
* La condizione di rendering assicurata restituisce un risultato sano se espressioneResolver è null.
...
* (Profilo metadati) I valori di tag standard impostati al momento della creazione non sono disponibili dopo il salvataggio. NPR-23370: Hotfix per CQ-4235458
* (Interfaccia touch) Impossibile spostare più risorse a causa di un errore JS. NPR-23395: Hotfix per CQ-4241279
...
* (Firefox/Chrome) Impossibile scaricare le risorse nella pagina Condivisione risorse. NPR-23963: Hotfix per CQ-4244391
...
* (Portale marchio) La ricerca nel predicato di directory deve essere filtrata fuori dalla replica. NPR-23292: Hotfix per CQ-4241385
...
* (Portale del marchio) Abilitate la creazione di risorse secondarie al momento della pubblicazione di una risorsa. NPR-23331: Hotfix per CQ-4242018
...
* I dati di EmbedXMP sono sempre impostati su “active” per il processo di generazione Ptiff. NPR-22903: Hotfix per CQ-4234498
...
* Problema relativo al modulo di avvio modifica/ricaricamento di video con codifica Dynamic Media. NPR-23237: Hotfix per CQ-4240260
...
* La scheda Miniature nella proprietà della pagina non funziona se si estende la finestra di dialogo. NPR-22844: Hotfix per CQ-4241474
...
* (Editor pagina) L’immagine su un componente Immagine scompare quando si apre di nuovo la finestra di dialogo. NPR-23156: Hotfix per CQ-4239978
...
* (Editor Rich Text) - L&#39;operazione di sostituzione modifica il formato di una parola. NPR-23271: Hotfix per CQ-4239677
...
* Dopo aver aggiunto all’incirca 80 frammenti in una cartella in Assets, si verificano degli errori quando si attiva il flusso di lavoro dalla console di Timeline. NPR-23393: Hotfix per CQ-4211216
...
* Errore &#39;Valore selettore ricorsione non valido&#39; durante la migrazione di un componente da AEM 6.0 a AEM 6.2. NPR-23532: Hotfix per CQ-4241258
* (Editor Rich Text) Le descrizioni comandi mostrano il nome variabile del plug-in invece del nome leggibile del plug-in. NPR-23550: Hotfix per CQ-4243269
...
* (Target) Le campagne non vengono attivate quando si utilizza il tipo di API come XML. NPR-23199: Hotfix per CQ-4240936****
...
* (OMEGA) Report &quot;Feature&quot; solo in inglese. NPR-23990: Hotfix per GRANITE-21231
...
* (ContextPath) La funzionalità di interruzione dei messaggi per le configurazioni e gli errori Jetty durante la ricerca dei membri del gruppo nell&#39;elenco dei membri del gruppo community. NPR-23336: Hotfix per CQ-4241519, CQ-4242080
* Il caricamento di un’immagine in un forum Communities non viene visualizzato in Provider risorsa di archiviazione Adobe (ASRP). NPR-23397: Hotfix per CQ-4242497
...
* Problemi durante l’esecuzione della   eseguite la ricerca nel componente forum. NPR-23838: Hotfix per CQ-4243770
* (Flag Moderazione community) L&#39;opzione Consenti in blocco dei messaggi contrassegnati non funziona. NPR-23845: Hotfix per CQ-4243962
* Il testo del pulsante Ordina non visualizza il   value, nonostante   selezionare l&#39;ordine di ordinamento predefinito. NPR-23881: Hotfix per CQ-4243375
...
* Se si allegano per due volte due file con lo stesso nome a un post di tipo tipo modulo, si verifica un errore del server. NPR-24166: Hotfix per CQ-4244367
...
* Durante il rollout della pagina, la Live Copy non mostra le proprietà di tempo di attivazione/disattivazione ereditate dal master. NPR-23873: Hotfix per CQ-4243431
...
* Richiesta di un meccanismo per ripulire le risorse dalle istanze del lavoratore dopo l&#39;elaborazione o su base regolare. NPR-23638: Hotfix per GRANITE-21337
...
* (FDM) (WebService) Supporto del costrutto di estensione di WSDL nel parser WSDL. NPR-23640, NPR-23236: Hotfix per 4205821
...
* Aggiornamento dei bundle   felix   bundle http per abilitare gli attributi di sessione.
...
* (Interfaccia touch) Posizione di caricamento predefinita per le risorse nella vista a colonne. NPR-22465: Hotfix per CQ-4237057
...
* (DAM) Più vulnerabilità di scripting tra siti (XSS) durante l&#39;aggiunta alla procedura guidata raccolta. NPR-22432: Hotfix per CQ-4327086
...
* Il nuovo elemento nella inbox Workflow mostra il percorso della pagina invece del titolo della pagina. NPR-21634: Hotfix per CQ-4230672
...
* (TouchUI) Più vulnerabilità di scripting tra siti (XSS) per i componenti HTL. NPR-21899: Hotfix per CQ-4232511
...
* (Interfaccia touch) L&#39;Editor Rich Text rimuove tutti gli attributi diversi dall&#39;ID dal tag &lt;a>. NPR-22044: Hotfix per CQ-4234133
...
* (Interfaccia classica) L’output dell’editor di testo RTF aggiunge &quot;list-position-style&quot;: inside;&#39; come stile in linea con il tag &lt;ul>. NPR-22145: Hotfix per CRTE-114
...
* (Interfaccia classica) La scelta rapida da tastiera del testo (Ctrl+V) non funziona per il componente Testo OOTB (Rich Text). NPR-22224: Hotfix per CQ-4236224
* Il filtro del campo tag   non funziona come previsto durante la digitazione del testo. NPR-22236: Hotfix per CQ-4236655
* (Editor pagina) Quando si incollano dati di testo nel componente Mappa immagine, il componente Testo viene incollato anche quando si incollano dati di testo nel componente Mappa immagine. NPR-22264: Hotfix per CQ-4236230
...
* Quando si esegue il lancio con più pagine, le pagine secondarie non vengono pubblicate nei server di pubblicazione se l’opzione “Includi le pagine secondarie” non è selezionata nella   primo livello di contenuto principale. NPR-22482: Hotfix per CQ-4237818
...
* Problemi relativo al componente Immagine a causa di uno spazio aggiuntivo nella   . NPR-22528: Hotfix per CQ-4238183
* Quando si apre il componente con la modalità   in modalità inline, i plug-in caricati in precedenza non sono visibili la seconda volta. NPR-22591: Hotfix per CQ-4236850
* Se si elimina un lancio in un lancio nidificato, i lanci secondari diventano orfani. NPR-22621: Hotfix per CQ-4202639
* (Barra laterale interfaccia classica) La scheda Flusso di lavoro è disabilitata quando la pagina è in fase di blocco del flusso di lavoro. NPR-22722: Hotfix per CQ-4237557
* Dopo aver riflesso un’immagine aggiunta nel componente immagine in una pagina, le modifiche non vengono salvate e nella pagina viene visualizzata l’immagine   l’immagine originale viene visualizzata sulla pagina. Il supporto per il rendering è stato aggiunto al componente core dell&#39;immagine tramite [https://github.com/Adobe-Marketing-Cloud/aem-core-wcm-components/pull/141](https://github.com/Adobe-Marketing-Cloud/aem-core-wcm-components/pull/141). NPR-22801: Hotfix per CQ-4221539
...
* Quando si attiva una pagina con istanze di   le istanze di pubblicazione disattivate generano lo stato verde invece di ruotare l’area in giallo. NPR-22927: Hotfix per CQ-4236310
...
* PersonalizationDataSource genera un’eccezione Null Pointer perché manca un nodo   jcr : content node. NPR-22850: Hotfix per CQ-4222122
* Il targeting AEM non riesce quando si utilizza una lingua   non inglese. NPR-22917: Hotfix per CQ-4218213
...
* Gli utenti non possono visualizzare i valori di test di Parametro statico nella chiamata a   chiamata mbox visibile quando si esegue il test con AT.js come libreria client nella configurazione cloud. NPR-21930: Hotfix per CQ-4234520
...
* Problemi di stabilità con la condivisione di risorse AEM&#39;istanza dell&#39;editore a causa di una perdita di memoria. NPR-22205, NPR-23178: Hotfix per Sling-5668, Sling-7292 e Sling-7470
...
* Quando   a http   la sessione gestita dalla lavagna è invalidata, la sessione contenitore viene anche invalidata se la sessione non dispone di altri attributi di sessione. NPR-23059: Hotfix per FELIX-5819
...
* Abilitazione della creazione del flusso di lavoro nel menu Frammenti esperienza. NPR-22347: Hotfix per CQ-4221661
* Errori di Frammenti esperienza riproducibili in WeRetail. NPR-21958: Hotfix per CQ-4220061
* Quando si attiva una pagina contenente un frammento esperienza eliminato, viene generata un’eccezione NullPointerException. NPR-23179: Hotfix per CQ-4239939
...
* (Interfaccia classica) L’attivazione o la disattivazione di un avvio di workflow comporta un comportamento non corretto. NPR-22907: Hotfix per CQ-4239153
...
* Le funzioni personalizzate per l&#39;editor di regole stanno aggiungendo un extra; dopo la chiamata della funzione, quindi, non è possibile eseguire la convalida anche se la funzione personalizzata restituisce true. NPR-22481: Hotfix per CQ-4235499
...
* (Microsoft Edge) (IE11) Il componente Campo di testo per modulo adattivo con campo multiriga visualizza &#39;Null&#39; come valore predefinito, invece di vuoto. NPR-22284: Hotfix per CQ-69107
...
* (Firefox) Impossibile aggiornare le proprietà dello schema XML di Forms adattivo, poiché le opzioni nel DOR (Document of Record) non vengono selezionate come già nella pagina delle proprietà. NPR-22298: Hotfix per CQ-4237402
...
* Discrepanza tra  Acrobat DC e AEM rapporti sull&#39;errore del controllo di conformità PDF/A-1b. NPR-22051, NPR-22050: Hotfix per CQ-4226128, CQ-4227671
...
* (Area di lavoro HTML) Problemi relativi alle dimensioni dello schermo con le barre di scorrimento. NPR-23288
* (Area di lavoro HTML) I punti di partenza del processo non sono ordinati in ordine alfanumerico. NPR-22841: Hotfix per CQ-4238944
* (Area di lavoro HTML) La funzione Prepara dati viene attivata alla chiusura del modulo nell&#39;area di lavoro. NPR-21127: Hotfix per CQ-4224574
* (Area di lavoro HTML) Quando si richiama un processo che richiede note con descrizioni lunghe, il pulsante di espansione Note non funziona. Hotfix per CQ-4241488
...
* Il caricamento di una risorsa come nodo figlio diretto di se stesso tramite l’interfaccia touch causa un problema. La risorsa viene caricata come elemento secondario diretto della risorsa precedentemente selezionata. NPR-19736
...
* Il difetto di metadati dell&#39;API delle risorse DAM interrompe le API xmp -write-back. NPR-20607: Hotfix per CQ-4220455
...
* Se il nome del video contiene spazi o accenti, non viene prodotto alcun video per la pagina Rappresentazione. NPR-19961: Hotfix per CQ-4221014
...
* I frammenti esperienza non vengono visualizzati correttamente in Internet Explorer 11. NPR-20161: Hotfix per CQ-4223319
...
* Il selettore utenti graniti OTB non carica alcun utente quando si fa clic sul menu a discesa nel passaggio del partecipante della finestra di dialogo del modello di workflow. NPR-20236
...
* Manca il supporto per l&#39;espressione di aggregazione per i pannelli ripetuti. NPR-20861
...
* Il profilo HTML viene visualizzato per un modulo adattivo anche quando il relativo tipo di risorsa non è XDP. NPR-20079
...
* Gestisci invio flusso di lavoro elenca solo dieci applicazioni. CQ-4230193

#### Moduli HTML5  {#html-forms-3}

* Il nome di un oggetto modulo XDP come il sottomodulo viene visualizzato come descrizione comandi quando non è definito nelle configurazioni di accessibilità. NPR-20523

### Programma di installazione di JEE per Forms  {#forms-jee-installer-9}

#### Gestione processi {#process-management-2}

* Il punto di avvio di FormSetPrefillApp non precompila i campi del set di moduli &#39;app AEM Forms. NPR-20950

#### Forms - AEM (LiveCycle)  {#forms-aem-livecycle}

* Installate la più recente libreria CTJPEG2K per una vulnerabilità di sicurezza critica. Questo interessa i moduli XMLFM (AEM e IfBA), RM e PDFG. NPR-20625, NPR-21337.
...
FileAEM Cumulative Fix Pack 6.3.1.1 è un aggiornamento importante che include diverse correzioni interne e per i clienti, a partire dalla disponibilità generale di AEM 6.3 Service Pack 1 (6.3.1.0) nell&#39;ottobre 2017.
...
* Le schede allegate, note e dettagli del flusso di lavoro non funzionano nell&#39;area di lavoro quando il modulo viene ingrandito o ridotto a icona e salvato come bozza o inoltrato. NPR-20243
...
* Correzioni relarive a diversi problemi di sovrapposizione dei progetti
...
* I componenti localizzati modificati non vengono ripristinati nella forma originale quando l&#39;ereditarietà viene riapplicata in LiveCopy. NPR-18172: Hotfix per CQ-4211379
...
* Correzioni relarive a diversi problemi di sovrapposizione dei progetti. NPR-17733
...
Le correzioni per AEM Forms vengono distribuite tramite il pacchetto di componenti aggiuntivi per Forms e altri programmi di installazione delle patch forniti con la versione. Per informazioni dettagliate, consultate [ AEM Forms Release](aem-forms-releases.md).
...
AEM Cumulative Fix Pack 6.3.0.1 è un aggiornamento importante che include diverse correzioni interne e per i clienti a partire dalla disponibilità generale di AEM 6.3 nell&#39;aprile 2017. Le principali caratteristiche del AEM Cumulative Fix Pack sono:

* Miglioramenti nell&#39;area seguente:

   * Frammenti di contenuto, Sites e componenti Editor Rich Text, Editor regole ed Editor modelli
   * Social Review e accesso a Facebook Social
   * Configurazione e avvio dei processi di traduzione
   * Tempo di risposta per l&#39;accesso alle notifiche in Social Communities
   * Visualizzazione delle attività di revisione nell&#39;archivio DAM

* Fornisce l&#39;opzione di convalida in Gestione pacchetti per rilevare i conflitti tra sovrapposizione e CFP

## Istruzioni per il download di CFP tramite distribuzione software {#download-instructions-for-cfp-via-package-share}

È possibile scaricare il pacchetto CFP direttamente da [Distribuzione software](https://www.adobeaemcloud.com/content/marketplace/marketplaceProxy.html?packagePath=/content/companies/public/adobe/packages/cq630/cumulativefixpack/AEM-CFP-6.3.3.8) oppure eseguire le operazioni seguenti:
...
>[!NOTE]
>
>Prima di installare il pacchetto, è consigliabile eseguire la convalida su ogni nuovo pacchetto di installazione. La pre-convalida analizza e segnala eventuali errori riscontrati prima dell&#39;installazione e segnala agli utenti tali errori in modo proattivo.
>
>È possibile accedere alla documentazione per l&#39;opzione Convalida all&#39;indirizzo [https://docs.adobe.com/content/docs/en/aem/6-3/administer/content/package-manager.html#Package%20Validator](https://docs.adobe.com/content/docs/en/aem/6-3/administer/content/package-manager.html#Package%20Validator)
...
* Per una distribuzione cluster tramite RDBMK o MongoDB, il pacchetto CFP può essere installato in una qualsiasi delle istanze Author che utilizza Package Manager.
...
* Potete aggiungere un nuovo logger nel percorso predefinito [http://localhost:4502/system/console/slinglog](http://localhost:4502/system/console/slinglog) con le seguenti proprietà:
...
activity.log verrà creato in   cartella crx -quickstart /logs.

### Installare Cumulative Fix Pack tramite la distribuzione del software {#install-the-cumulative-fix-pack-via-package-share}
...
* Utilizzate l&#39; [API HTTP da Package Manager](https://helpx.adobe.com/experience-manager/6-3/sites/administering/using/package-manager.html) - accertatevi di utilizzare `cmd=install&recursive=true` - per installare il pacchetto nidificato.

### Convalidare l’installazione {#validate-installation}

1. La pagina Informazioni prodotto ( `/system/console/  productinfo`) ora deve mostrare la stringa di versione aggiornata &quot;Adobe Experience Manager, versione 6.3.3.8&quot; in Prodotti installati.
1. Tutti i bundle OSGi risultano come ATTIVI o FRAMMENTI nella console OSGi (usa la console Web: `/system/console/bundles`).

>[!NOTE]
>
>Dopo l&#39;installazione corretta del pacchetto, nei registri di errore viene visualizzato un messaggio informativo che indica che il pacchetto di contenuto è stato installato correttamente, ad esempio &quot;Pacchetto di contenuto **AEM-6.3.3-Cumulative-Fix-Pack-7** installato correttamente&quot;.

>[!NOTE]
>
>A partire da AEM 6.3.3.1, il livello registro in Jackrabbit FileVault è cambiato da INFO a DEBUG per la maggior parte dei messaggi. Per ottenere i registri di installazione per un pacchetto di contenuti installato AEM 6.3.3.1 o versione successiva, abilitate il livello di registro DEBUG per org.apache.jackrabbit.vault.package.impl.
...
1. Installare il pacchetto del componente aggiuntivo Forms come descritto in [Installazione di pacchetti del componente aggiuntivo AEM moduli](https://helpx.adobe.com/experience-manager/6-3/forms/using/installing-configuring-aem-forms-osgi.html).
...
1. Per installare l&#39;aggiornamento, eseguire il file Designer 6.2.0_&lt;Lingua>_Cumulative_QF.msp.
...
* Durante l&#39;installazione di AEM CFP 6.3.3.x possono verificarsi i seguenti errori e avvisi e possono essere ignorati in modo sicuro:

   * *WARN* [OsgiInstallerImpl] org.apache.jackrabbit.vault.impl.InstallHookProcessorImpl Hook /META-INF/vault/hooks/cloudservices-wfchangeinstallgancio-0.0.2-jar-with-dependencies.jar ha generato un&#39;eccezione runtime.
   * *ERRORE* [OsgiInstallerImpl] com.adobe.cq.social.cq-social-jcr-provider [com.adobe.cq.social.provider.jcr.impl.SpiSocialJcrResourceProviderImpl(2174)] Timeout in attesa del completamento della modifica reg. CQ-4209974.
   ...
   * com.day.cq.wcm.mobile.core.MobileUtil isMobileResource: impossibile controllare la risorsa [/bin/receive], gestione pagina non disponibile
   ...
   * *ERRORE* [FelixDispatchQueue] com.day.cq.dam.cq-dam-core FrameworkEvent ERROR (org.osgi.framework.ServiceException: Service factory ha restituito null. (Component: com.day.cq.dam.handler.standard.ps.PostScriptHandler))
...
>[!NOTE]
>
>Questa impostazione di configurazione si applica al CFP 6.3.3.2 e versioni successive. Viene indicato di aggiornare le proprietà di delega di avvio nel file `sling` .properties.

Per aggiornare manualmente le modifiche in adobe- livecycle - cq -author.ear/ cq.war, procedere come segue:
...
* Apri adobe-livecycle-cq-author.ear/cq.war/WEB-INF/web.xml per la modifica:
   ...
   * aggiorna il valore **sling.bootdelegate.ibm** param-name con:
...
* Disinstallare il file dell&#39;archivio enterprise precedente (EAR) dal server dell&#39;applicazione della websfera e installare il file EAR aggiornato seguendo la procedura descritta nella sezione 10.2 di [https://helpx.adobe.com/pdf/aem-forms/6-3/install-single-server-websphere.pdf](https://helpx.adobe.com/pdf/aem-forms/6-3/install-single-server-websphere.pdf)
...
>[!NOTE]
>
>Questa impostazione di configurazione si applica alla versione 6.3.2.2 e alle versioni successive. Viene indicato di aggiornare manualmente i criteri degli elenchi di controllo degli accessi tramite CRX-DE, in quanto gli ACL non vengono aggiornati tramite l&#39;installazione di CFP a causa di &quot;merge_preserve&quot; acHandling.
...
`a)` Principal : servizio di regolazione\
Tipo: Consenti\
Privilegi: jcr:read , jcr:editProperties\
Restrizioni : rep:idspn=&quot;/*/jcr:content&quot;\
`b)` Principal : servizio di regolazione\
Tipo: Consenti\
Privilegi: jcr:read , jcr:editProperties\
Restrizioni : rep:DSM=&quot;/*/jcr:content/*&quot;
...
La proprietà controlla la profondità massima della sottostruttura del nodo sotto il nodo ` /  jcr   :content` della pagina, fino alla quale i nodi presenti nella directory archivio verranno utilizzati per ottenere le proprietà della pagina. Qualsiasi nodo presente sotto la profondità specificata in questa proprietà viene ignorato.

Il valore predefinito è 1. Il valore può essere   per ignorare, sovrapporre la proprietà file costanti.js (`/libs/cq/ui/widgets/source/constants.js`) rimuovendo il commento dalla proprietà CQ.PAGE_PROPERTIES_MAX_RECURSION_LEVEL e assegnando il valore richiesto (la profondità massima sotto la proprietà / jcr :content della pagina fino alla quale vengono memorizzati i dati delle proprietà pagina).

**Se l&#39;utente deve creare più varianti di pagine in modo che il numero di nodi sotto il nodo jcr:content della pagina diventi maggiore di 1000, procedere come segue per apportare modifiche alla configurazione:**

* Configurare la proprietà Max results JSON di Apache Sling
* Get Servlet using `/system/console/  configMgr`
* Impostate il relativo valore su un numero >1000 (valore predefinito corrente) in modo che questo numero sia maggiore del numero totale di nodi nella sottostruttura / jcr :content fino alla profondità configurata sopra.

Questo consente al servlet del GET sling di restituire tutti i nodi richiesti.

## JAR Uber {#uber-jar}

La Jar Uber per 6.3.3.8 è disponibile in [ repository Public Maven in Adobe](https://repo.adobe.com/nexus/content/groups/public/com/adobe/aem/uber-jar/6.3.3.8/).

Per utilizzare Uber Jar in un progetto Maven, fare riferimento all&#39;articolo [Come utilizzare Uber jar](https://helpx.adobe.com/experience-manager/6-3/sites/developing/using/ht-projects-maven.html) e includere nel POM del progetto la seguente dipendenza:
...
>[!MORELIKETHIS]
>
>* [Versioni e aggiornamenti per AEM](https://helpx.adobe.com/it/experience-manager/aem-releases-updates.html)
...
>* [Documentazione di AEM 6.3](https://docs.adobe.com/content/docs/en/aem/6-3.html)
