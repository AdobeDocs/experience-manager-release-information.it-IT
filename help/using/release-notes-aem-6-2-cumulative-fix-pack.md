...
AEM Cumulative Fix Pack 6.2 SP1-CFP20 è l’ultimo Cumulative Fix Pack per AEM 6.2 e costituisce un aggiornamento di rilievo contenente importanti correzioni per la clientela, introdotte successivamente alla data di disponibilità generale di [AEM 6.2 SP1](https://experienceleague.adobe.com/it/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).

>[!CAUTION]
>
>L’applicazione di CFP senza la convalida di compatibilità tra i Feature Pack installati, potrà causare un errore di sistema o la perdita delle configurazioni personalizzate, che potrebbe richiedere l’esecuzione del ripristino dal backup per risolverli.

>[!NOTE]
>
>* Con AEM Cumulative Fix Pack 6.2 SP1-CFP10 viene fornito un nuovo bundle `discovery- api` Sling Johnzon 1.0.0. È inoltre disponibile una funzione di rilevamento sling per l’utente del servizio con privilegi di lettura e scrittura relativi al nodo */var/discovery* nell’archivio CRX.
>
...
>[!NOTE]
>
>Per la clientela di AEM Forms, è essenziale installare il pacchetto dei componenti aggiuntivi per AEM Forms, dopo l’installazione del Service Pack, del Cumulative Fix Pack o del Feature Pack di AEM.
...
* Quando si visualizza l’anteprima della lettera come PDF, il campo di testo inserito nella pagina principale non rispetta il valore immesso nella scheda dati o secondo i collegamenti dati specificati. NPR-29239: Hotfix per CQ-4266856.
...
* Impossibile rinominare una cartella DAM se manca l’autorizzazione Modifica in ACL. NPR-27555: Hotfix per CQ-104652
* Mancata risposta dello strumento di editor predefiniti immagine in CFP 17 6.2.1 e versioni successive. NPR-28147: Hotfix per CQ-4261041
...
* La visualizzazione dell’icona dell’interruzione dell’ereditarietà non è coerente. NPR-27671: Hotfix per CQ-4257779
...
* È stato aggiunto il supporto per il reindirizzamento al componente pagina core.
...
* La funzionalità di download utilizza AEM Assets tramite il servlet di download delle risorse, consentendo agli utenti anonimi di scaricare tutte le risorse. NPR-27054: Hotfix per CQ-4254732
...
* A causa di un comportamento errato della classe ConfigPostProcessor, la sospensione dell’immagine principale rimuove il tipo mixing `cq:LiveRelationship` dalla pagina secondaria. NPR-26745: Hotfix per CQ-4254163
* Aggiungi il supporto per il reindirizzamento al componente pagina core. NPR-26576: Hotfix per CQ-110529
...
* Per alcune pagine, l’amministrazione sito classica non esegue il rendering dell’elenco nel riquadro a destra. NPR-27247: Hotfix per CQ-4251621
...
* A causa della presenza del carattere speciale trattino lungo, l’editore sta riscontrando un problema nello scaricamento della cache. NPR-27199: Hotfix per CQ-4242790
...
* Il pacchetto di componenti aggiuntivi per AEM Forms non contiene nuove correzioni.
...
* Utilizzando l’integrazione Target integrata, il targeting di un componente riproduce l’intera pagina e non un componente di destinazione vuoto. NPR-25273: Hotfix per CQ-4248003
...
* La variabile viene compilata come tipo di dati Elenco e il sottotipo è una stringa, ma viene visualizzato l’errore “impossibile assegnare l’oggetto”. NPR-26194: Hotfix per CQ-4252287
...
* Quando un componente è destinato a un’istanza di pubblicazione, uno sfarfallio rende visibile l’esperienza predefinita prima di quella di destinazione. NPR-23992: Hotfix per CQ-4242038
* Le esperienze eliminate dall’ambiente di authoring non vengono mai rimosse dal sito pubblicato al momento dell’attivazione della pagina. NPR-24869: Hotfix per CQ-4247832
...
* LineChecker non riesce a esternalizzare i collegamenti se una richiesta contiene parametri di query non validi. NPR-24127: Hotfix per CQ-4241893

### MSM {#msm}

* Hotfix proattivi per attacchi di vulnerabilità cross-site scripting (XSS). NPR-21893: Hotfix per CQ-4223385
...
* Un’azione di pubblicazione su una pagina bloccata interrompe l’authoring. NPR-23970: Hotfix per CQ-4243203
...
* La creazione di un lancio non riesce quando si avvia il flusso di lavoro “Richiesta lancio” in un progetto. NPR-24255: Hotfix per CQ-4245030
...
* (Interfaccia utente classica) Il componente mostra i valori predefiniti anche se il servizio del modello dati modulo associato è impostato su un campo vuoto. NPR-21903: Hotfix per GRANITE-19744
* Più campi genera un’eccezione NPE se per la richiesta non sono disponibili FormData. NPR-24513: Hotfix per GRANITE-21055
...
* Quando si esegue l’anteprima predefinita di una lettera, la mappatura dei frammenti di layout non viene visualizzata; se si fa clic su Anteprima, appare invece correttamente. NPR-22993: Hotfix per CQ-4237745
...
* Sono state effettuate correzioni proattive della sicurezza nella tabella di Foundation per mantenere un design coerente.
...
* È stato eseguito l’aggiornamento alla versione più recente del file adobe-reader-extensions-dsc.jar per introdurre alcune correzioni nell’estensione Reader.
...
* I dati EmbedXMP sono sempre impostati su “attivi” per il processo di generazione piramidale di TIFF. NPR-22776: Hotfix per CQ-4234498
...
* Se nella configurazione di traduzione viene impostata la risorsa “Non tradurre”, vengono comunque create delle copie in lingua della risorsa. NPR-22540: Hotfix per CQ-4237962
...
* Il componente CAPTCHA è ora obsoleto per una maggiore sicurezza. Se si utilizza il componente CAPTCHA, dopo l’installazione di 6.2 SP2-CFP15 o versione successiva viene visualizzato il messaggio “Il componente Captcha è obsoleto e non deve essere utilizzato”. Per maggiore sicurezza, si può personalizzare il componente AEM in modo da includere reCAPTCHA. NPR-22151: Hotfix per CQ-4220052
...
* Quando si esegue l’anteprima predefinita di una lettera, la mappatura dei frammenti di layout non viene visualizzata; se si fa clic sul pulsante Anteprima, appare invece correttamente. NPR-23335: Hotfix per CQ-4237745
* Nella lettera corrispondente alle associazioni definite in XDP, i dati non vengono aggiunti tramite l’URL della lettera diretto. NPR-24145: Hotfix per CQ-4244290
...
* I moduli incorporati in Sites non vengono pubblicati quando si ripubblica la pagina del sito. NPR-23014: Hotfix per CQ-4236566
...
* La presenza di virgolette singole impedisce l’aggiornamento dei metadati nella vista in blocco e l’interfaccia risulta danneggiata quando si aprono le proprietà di una risorsa mediante le azioni rapide della barra degli strumenti. NPR-22317, NPR-22353: Hotfix per CQ-4236990, CQ-4236469
...
* (Interfaccia Touch) Impossibile visualizzare più di 40 tag per uno spazio dei nomi. NPR-22290: Hotfix per CQ-99114
* Problemi relativi ai feed RSS, porta -1 a AEM 6.2. NPR-22158: Hotfix per CQ-4233339
* (IE) Quando si esegue l’authoring di un carattere qualsiasi in un campo di testo formattato per la prima volta, al carattere viene aggiunto uno spazio finale. NPR-22443: Hotfix per CQ-4235343
* Quando si tenta di trovare una corrispondenza per il nome pacchetto, l’oggetto Use Java™ blocca il SightlyJavaCompilerService a causa di uno spazio finale nella dichiarazione del pacchetto. NPR-22557: Hotfix per GRANITE-20836
...
* (Digital Publishing Suite) Prima di caricare i folio in DPS, è necessario impostare i campi sia della data di pubblicazione che di quella di copertina. NPR-22484

### Commerce {#commerce}

* Più vulnerabilità cross-site scripting (XSS) nella procedura guidata di Crea cataloghi in Commerce. NPR-22344: Hotfix per CQ-4237017
...
* Se si esegue il rollout di una struttura spostata in un lead principale, `cq:moveTarget` non viene eseguito correttamente. NPR-22373: Hotfix per CQ-4232536
...
* Il motore Target (mbox.js, at.js) non utilizza URL gestiti e utilizza URL contenenti due punti, che potrebbero generare errori con determinate implementazioni. NPR-22366: Hotfix per CQ-4237854
...
* Il flusso di lavoro Starter non è in grado di accettare il valore di stringa TypeHint. NPR-22436: Hotfix per CQ-4237855
...
* (Workspace HTML) Quando un utente richiede un’attività, il conteggio della coda viene aggiornato solo per quel particolare utente. A meno che la pagina non venga aggiornata. NPR-19778: Hotfix per CQ-4233665
...
* Nelle impostazioni del componente Target, è stata abilitata la configurazione del campo Parametro statico quando si utilizza AT.js come libreria client.
* Sono state apportate correzioni alla funzionalità Mostra/Nascondi del componente del menu a discesa.
...
* (Safari) Gli utenti non riescono a visualizzare tutte le risorse nel pannello Trova risorsa. NPR-21981: Hotfix per CQ-4213720
* Il Timewarp restituisce l’errore “RecursionTooDeepException” con una pagina illeggibile e non viene creata una nuova versione, anche se la data viene modificata. NPR-21707: Hotfix per CQ-4199536
* Durante il caricamento di una pagina nell’editor, WorkflowStatusprovider (pageinfo.json) viene caricato tre volte, rallentando le prestazioni dell’istanza di AEM. NPR-21778: Hotfix per CQ-59232
...
* Gli utenti non vengono sincronizzati utilizzando la relativa sincronizzazione con la distribuzione Sling. NPR-21911: Hotfix per GRANITE-20404
...
* Quando si aggiunge un framework Target, la modalità di targeting non è disponibile nell’elenco Seleziona modalità nell’editor. NPR-21047
...
* Il processo di ripristino di una Live Copy viene interrotto dopo l’eliminazione della prima pagina locale in LC. NPR-21276: Hotfix per CQ-4229743

### Platform {#platform-5}

* La libreria Tag personalizzata che fa riferimento a tag implementati come script non viene trovata dopo l’aggiornamento ad AEM 6.3. NPR-20149: Hotfix per GRANITE-18433
...
* HTTP.encodePathOfURI espande due volte la codifica del segno più “+”. NPR-21417: Hotfix per GRANITE-16340
...
* Il pulsante di invio dell’anteprima di una lettera risulta disabilitato dopo l’installazione di AEM 6.2 SP1-CFP9. NPR-21547
* Quando si seleziona il tipo di collegamento Risorsa, la finestra del selettore risorse non si apre nella procedura guidata del binding dei dati per Modifica lettera. NPR-21164: Hotfix per CQ-4194567
...
* L’aggiornamento a Java™ 8 131 più recente genera un’eccezione: “Il provider JsafeJCE è disattivato, non è stato possibile effettuare la verifica dell’autointegrità FIPS 140 richiesta”. NPR-21355
...
* Aggiornamento dei file JAR di Jsafe a CryptoJ 6.1.3.1 nei servizi Core, Crittografia, Firma e Sicurezza documento. NPR-21360, NPR-21361, NPR-21356, NPR-21358
...
* Sono state introdotte correzioni nel componente Sightly e LiveCopies.
...
* Il collegamento di un file di risorse codificato con l’apposito plug-in nell’editor Rich Text restituisce un collegamento vuoto. NPR-20626, NPR-21059: Hotfix per CQ-4223011
* L’editor di metadati del frammento di contenuto impedisce agli autori dei contenuti di salvare le modifiche apportate ai frammenti di contenuto. NPR-20641: Hotfix per CQ-4224755
...
* (Interfaccia utente per la corrispondenza) Dopo un clic, il pulsante Invia viene disabilitato. NPR-21078

### Programma di installazione di Forms per JEE {#forms-jee-installer-9}

#### Servizio assemblatore {#assembler-service-1}

* docConvertor non riesce a produrre PDF/A con l’errore “Il prefisso “stEvt” per l’elemento `stEvt:action` non è associato”. NPR-21032: Hotfix per CQ-4222540
* Mentre si richiama il servizio OMPFSubmission/PDFA/PDFtoPDFA viene generata un’eccezione con il nome `java.lang.IllegalArgumentException message:No enum constant com.adobe.internal.pdfm.docbuilder.signature.PathValidationFailureReason.SIGNED_IN_FUTURE`. Questa eccezione impedisce che il processo di verifica della firma di breve durata possa essere completato, fino al riavvio del server. NPR-20792
...
* Il processo di preparazione dei dati viene richiamato più volte quando si esegue il rendering del modulo per dispositivi mobili con parametri dataref. NPR-19801: Hotfix per CQ-4230427, CQ-4230400
...
* Le risorse Miniatura il cui titolo contiene dei caratteri speciali mostrano l’oggetto Java™ nell’attributo alt di NPR-20347: Hotfix per CQ-4223620
...
* Se il filtro data modificato viene cambiato dopo l’apertura della ricerca salvata, i risultati non vengono influenzati. I risultati visualizzati sono identici al precedente valore salvato del filtro data modificato. NPR-19739: Hotfix per CQ-4219425
...
* (Interfaccia touch) L’eliminazione dei lanci tramite la console dell’interfaccia utente classica rende tutte le pagine non modificabili. NPR-20520: Hotfix per CQ-4225074
...
* Non viene visualizzato alcun messaggio a comparsa di conferma o di reindirizzamento quando si creano prodotti dalla stessa pagina. NPR-20257: Hotfix per CQ-4223414
...
* La proprietà Mappa immagine esiste ancora anche dopo l’eliminazione delle immagini dal componente Editor locale di più immagini. NPR-20142: Hotfix per CQ-4222982
...
* Quando modificato tramite l’interfaccia, lo script valueCommit viene eseguito due volte per DropDownList. NPR-19989: Hotfix per CQ-110212
...
* La finestra di dialogo di selezione dei certificati di autenticazione reciproca/CAC dovrebbe visualizzare i certificati con Utilizzo chiave privata (EKU) per l’accesso con autenticazione client o smart card. NPR-20708
...
* Sono state apportate correzioni alla cache di persistenza indipendente per Context Hub.
...
Questa esperienza fornisce una nuova versione di Dynamic Media che si collega alla nuova infrastruttura di pubblicazione di Dynamic Media per una replica più rapida e scalabile.
...
* AEM mostra un comportamento imprevisto quando un utente tenta di pubblicare la versione `/unpublish/create` di una pagina bloccata da un altro utente. NPR-19249: Hotfix per CQ-4215298 e CQ-4203856
* Durante la promozione manuale del lancio nidificato, la pagina secondaria viene eliminata. NPR-19704
...
* Abilitazione alla pubblicazione di tag da AEM a Brand Portal dalla console tagadmin/assegnazione tag. NPR-20271
...
* Il pulsante Invia viene disabilitato dopo un clic per impedire l’invio di più moduli in una pagina di traduzione.
...
* Problemi durante l’applicazione del filtro Omnisearch alle raccolte avanzate nel browser Safari. NPR-19511
...
* Il servizio di notifica dei flussi di lavoro non codifica i collegamenti nel messaggio e-mail, impedendone il caricamento dopo essere stati selezionati dagli utenti. NPR-19490: Hotfix per CQ-4218055
...
* Quando si attiva il flusso di lavoro “Richiedi attivazione”, viene visualizzata un’icona di disattivazione errata nella casella in entrata AEM. NPR-19365: CQ-4216174
* Problemi di ordinamento nella vista a elenco. NPR-19217: CQ-95602
* Quando si modifica il titolo o l’immagine in miniatura nelle impostazioni della cartella delle risorse, il gruppo e le autorizzazioni originali della cartella vengono ignorati. NPR-19283: Hotfix per CQ-4216080
* Le workstation `Windows 10` passano automaticamente alla modalità touch, disattivando alcuni pulsanti. NPR-19183

### Sites {#sites-11}

* Si verificano problemi in presenza di più componenti dell’editor Rich Text in una finestra di dialogo. NPR-19311: NPR-19587
* La funzionalità automatica di Pulizia delle versioni funziona in Vanilla AEM 6.2 solo dopo aver inizializzato VersionManagerImpl. NPR-19315: Hotfix per CQ-4217175
...
* Gli editor di progetti non possono copiare/incollare risorse nella cartella risorse del progetto. NPR-19619
...
* La finestra di dialogo Targeting non viene visualizzata nella modalità di targeting quando un modulo di destinazione viene modificato e salvato più volte. NPR-19144: Hotfix per CQ-4216708

### Flusso di lavoro {#workflow-2}

* Nell’interfaccia utente classica, gli utenti non possono filtrare le notifiche in Casella in entrata per utente/gruppo. NPR-19122: Hotfix per CQ-4215374
...
>* La ricerca non funziona con eventuali contenuti dinamici (come valori di elementi del dizionario dei dati o valori di variabili) all’interno del frammento di documento.

#### Forms Manager {#forms-manager-2}

* Le proprietà dello Schema XML nei moduli adattivi non possono essere modificate dopo aver applicato CFP6 in AEM 6.2. Hotfix per CQ-4219684
* Tutti i servizi del bundle di base della gestione di AEM Forms non vengono avviati al riavvio del server. Hotfix per CQ-4217014
...
* Sono stati risolti alcuni problemi relativi alle prestazioni con Dynamic Media Cloud Services, all’interfaccia utente touch e di sicurezza.
...
* È stato aggiunto il supporto per il file della libreria Target AT.JS, una libreria di implementazione per l’integrazione lato client con Adobe Target progettata sia per le implementazioni web tipiche, sia per le applicazioni a pagina singola.
...
* I pulsanti dell’azione Pubblica sono visibili anche se l’utente non ha accesso alla replica. NPR-17353: Hotfix per CQ-4209269
...
* In un cluster di autoring attivo-attivo con MongoDB, entrambe le istanze di authoring tentano di attivare la replica per lo stesso contenuto quando viene raggiunta l’ora impostata per la pubblicazione. NPR-18708: Hotfix per CQ-4210982
...
* Le mappe delle immagini non mantengono le coordinate nel componente immagine HTL. NPR-18530: Hotfix per CQ-4211584
...
* Abilitazione alla pubblicazione di tag da AEM a Brand Portal dalla console tagadmin/assegnazione tag. CQ-4212165
...
* Se esiste una dichiarazione XML in un file dati XML e il rendering della lettera viene avviato tramite una richiesta POST, nella lettera corrispondente i dati non vengono visualizzati. NPR-18870
...
>
>* Quando si esegue un’operazione Copia/Incolla da Word o dal web nell’editor di testo di Gestione corrispondenza, i contenuti presentano interruzioni di riga. NPR-19530
>* I contenuti senza interruzioni di riga non vanno a capo nell’editor di testo di Gestione corrispondenza. NPR-19449
>

#### Moduli adattivi {#adaptive-forms-9}

* Nel caso di pannelli ripetibili, l’aggiunta di un nuovo pannello determina la cancellazione del valore del campo a discesa nel pannello precedente. NPR-18772
...
* Lo script per modificare il titolo del pulsante per l’evento di inizializzazione del pannello principale “guide” non funziona. NPR-18476
...
* Gestione efficiente dei componenti nascosti in modalità layout su tablet.
* Introduzione alle azioni rapide su dispositivi ibridi.
* Risoluzione dei problemi di sincronizzazione a livello di componente con le Live Copy.

### Risorse {#assets-13}

* Un cliente viene bloccato quando un utente che non dispone dell’autorizzazione necessaria tenta di eseguire l’operazione Sposta su una risorsa. NPR-18330: Hotfix per CQ-4212560
...
* I flussi di lavoro e le azioni di checkout non sono disponibili dalla console della timeline quando alla cartella delle risorse vengono aggiunti circa 80 frammenti. NPR-18257: Hotfix per CQ-4211214 e NPR-18251; Hotfix per CQ-4211216.
...
* La miniatura del PDF non viene generata. NPR-17831, NPR-17750: Hotfix per CQ-4210547
...
* Quando la regola di segmentazione è impostata per un intervallo IP, la personalizzazione funziona solo per il primo IP. NPR-18121: Hotfix per CQ-83767
...
* Le pagine eliminate contrassegnate sono visibili nell’interfaccia Touch. NPR-18025: Hotfix per CQ-86694
...
* Non è possibile selezionare un’immagine dopo la rimozione di un’altra immagine nella pagina. NPR-17711: Hotfix per CQ-4201323

### Platform {#platform-11}

* I controlli dell’interfaccia Touch risultano nascosti per gli utenti che dispongono delle autorizzazioni necessarie. NPR-17945: Hotfix per CQ-4211231
...
* Si verifica un attraversamento delle query quando nella blueprint del catalogo non è stata definita alcuna condizione per una sezione. NPR-18229; Hotfix per CQ-4211924
...
* Richiesta di utilizzare una versione aggiornata del servlet org.apache.sling.servlets.post (2.3.22) nell’API Apache Sling per prevenire una vulnerabilità XSS. NPR-18963

### Traduzione {#translation-6}

* L’invio della pagina della risorsa non deve essere più necessario per un progetto di traduzione fino al completamento del progetto. NPR-18249: Hotfix per CQ-4209908

### WCM - Componenti Foundation {#wcm-foundation-components-8}

* Non è possibile utilizzare il componente Iparsys di WCM Foundation nei modelli modificabili. NPR-18223; Hotfix per CQ-4210384
* La mappa immagine non mantiene le coordinate selezionate nel componente immagine HTL. NPR-18032; Hotfix per CQ-4211584
* Quando viene eseguito il rendering di un componente immagine HTL, il nome file nell’URL viene rinominato e l’URL non funziona più. NPR-17908; Hotfix per CQ-4211587
* Non è possibile uscire da Proprietà pagina dopo aver apportato delle modifiche. NPR-17832; Hotfix per CQ-96110
...
* Il dizionario dei dati viene letto ripetutamente durante il rendering delle lettere. NPR-18482: Hotfix per CQ-4210805
...
* È stata migliorata la reattività della barra delle azioni durante la visualizzazione delle azioni nella barra degli strumenti e dei componenti Coral.
...
* Quando si sposta una pagina usando l’interfaccia utente classica o touch, nella finestra di dialogo Sposta vengono visualizzati solo i primi 150 riferimenti; pertanto per gli utenti non è possibile aggiornare gli altri riferimenti e ripubblicare la pagina. Questo problema è stato risolto introducendo una proprietà per l’interfaccia classica: “maxRefNo”, che può essere configurata nel nodo di amministrazione del sito: `'/libs/wcm/core/content/siteadmin'`. Questa proprietà specifica il numero massimo di riferimenti (valore predefinito: 150) visualizzati prima di una grande operazione di spostamento. Se una pagina contiene diversi riferimenti, questi non vengono visualizzati nella finestra di dialogo Sposta pagina. Questa configurazione può essere utilizzata anche per amministratore DAM e amministratore MISC applicando la configurazione sui nodi: `'/libs/wcm/core/content/damadmin'` e `'/libs/wcm/core/content/miscadmin'` rispettivamente. NPR-17222: Hotfix per CQ-85878
...
* Per un modulo adattivo con allegato, le voci duplicate per i tag afSubmissionInfo vengono create nel codice XML inviato durante l’invio del modulo per la seconda volta. NPR-17364
* Se si utilizza il browser Google Chrome e, dopo aver rimosso un allegato da un modulo, si tenta di ricollegare lo stesso allegato, viene generato un errore. NPR-17297
...
* Il metodo API `com.adobe.icc.services.api.LetterInstanceService` per l’importazione non funziona. NPR-17922, NPR-16008
...
* L’interfaccia di gestione della corrispondenza non viene avviata se l’azione di invio HTML utilizza il metodo POST. NPR-17595

**Forms Manager**

* Con un modulo adattivo configurato per l’AB testing, facendo clic su Avvia AB testing, il test non viene avviato e viene generato un errore nella console del browser. NPR-17838

**Servizio Forms**

* Sono stati risolti i problemi segnalati nell’analisi del codice statico dei moduli OSGI. NPR-13951

#### Programma di installazione di Forms per JEE {#forms-jee-installer-15}

**Servizio di output**

* L’utilizzo del servizio di output di AEM Forms 6.2 per unire un modulo specifico a un XML di dati richiede un tempo 20 volte superiore. Ora è stato risolto negli ambienti Windows e Linux®. NPR-17501
...
* Che si utilizzi la console dell’interfaccia di amministrazione JEE o la console OSGi, le funzionalità degli utenti di Forms dovrebbero essere le stesse. NPR-17670
...
* Durante l’anteprima lettera, nella modifica di frammenti nell’editor di testo, deve essere visualizzato il testo elaborato per la modifica anziché le condizioni in linea utilizzate nei frammenti. NPR-15748, NPR-17504
...
* La personalizzazione o l’aggiunta di colonne ai risultati **OmniSearch delle risorse** tramite sovrapposizione in `/apps` non funziona. NPR-16737: Hotfix per CQ-4206785
...
* Il numero di risultati visualizzati nella schermata Gestione utenti è diverso nella vista a scheda e in quella a elenco. NPR-16241; Hotfix per GRANITE-16914
* Il caricamento lento delle notifiche del flusso di lavoro ha esito negativo durante la visualizzazione nel browser Google Chrome in modalità Schermo intero. NPR-17013: Hotfix per CQ-4207567
...
* I file `Camera RAW` (inclusi ARW, CR2, NEF, DNG ed EPS) non possono essere selezionati o eliminati quando vengono aggiunti come rappresentazioni nelle risorse. Tali file vengono scaricati automaticamente quando l’utente vi fa clic sopra. NPR-16949: Hotfix per CQ-4206846
* Se si crea un file PDF in un altro PDF nell’interfaccia di Assets, i file PDF creati non vengono visualizzati nell’interfaccia di DAM. Tuttavia, i PDF sono visibili nell’archivio CRX. NPR-16833: Hotfix per CQ-4206501
...
La console del flusso di lavoro Progetti mostra l’eccezione NullPointerException nella pagina, quando vengono eliminati flussi di lavoro. NPR-17017: Hotfix per CQ-4194269
...
* In WCM Launches, non viene annullata la pubblicazione delle pagine associate a un lancio se l’azione viene avviata dalle proprietà della pagina nell’interfaccia Touch. NPR-16560: Hotfix per CQ-4204681
...
* In WCM-Design Importer, l’apertura di una pagina di importazione di test e il tentativo di caricare un file ZIP causano problemi, se il file è già stato caricato ed eliminato in precedenza. NPR-16486: Hotfix per CQ-90962
* Il passaggio al pannello **[!UICONTROL Navigazione globale]**, che utilizza un browser Firefox o Google Chrome, offre un’esperienza utente diversa. Il browser Firefox visualizza il menu **[!UICONTROL Strumenti]**. Il browser Chrome di Google mostra il menu **[!UICONTROL Navigazione]**. NPR-16770; Hotfix per CQ-4200456

### Campaign {#campaign}

* Durante l’esecuzione del test dei modelli di AEM Campaign e la modifica degli indirizzi iniziali per includere “dati aggiuntivi”, l’elenco a discesa di Adobe Campaign scompare nel componente Context Hub dell’interfaccia Touch. NPR-16771: Hotfix per CQ-105748

### Mobile On-Demand {#mobile-on-demand-3}

* Durante la verifica preliminare di una pubblicazione dall’ambiente di authoring AEM, se un’azione di verifica preliminare dura più di cinque secondi, nella dashboard dell’integrazione AEMM - AEM PECS Splunk viene generato un picco insolito con un numero elevato di richieste di stato al secondo. NPR-16908: Hotfix per CQ-4207055
...
* La copia in lingua creata con lo strumento di traduzione punta alla directory principale anziché alla Live Copy locale. NPR-17257: Hotfix per CQ-4208287
...
* Nell’editor dei moduli adattivi, il commento di impostazione della destinazione in head.jsp deve essere sostituito con la nuova istruzione per Context Hub. NPR-17173
...
* Durante l’invio di un modulo adattivo tramite AJAX, la funzione di callback di “errore” non viene mai richiamata in caso di errore. NPR-17034
...
* Supporto per espressioni regolari di tipo `dataRef=C:/Users/` in **[!UICONTROL Configurazione predefinita servizio precompilazione]**. Questo tipo è per la precompilazione dei dati di Adaptive Forms. NPR-16425
...
* Nell’elenco a discesa Font non è disponibile la famiglia dei font quando si crea un modulo di testo basato sul dizionario dei dati tramite Internet Explorer. NPR-16944
...
#### Programma di installazione di Forms per JEE {#forms-jee-installer-16}

**Core**

* Durante il processo di aggiornamento, il gestore di configurazione ha esito negativo con errore di timeout. NPR-16774 (vedere [Configurazione del timeout per le operazioni a livello di componente](install-cfp-aem-forms-jee.md#configuring-timeout-for-operations-at-component-level-npr)).
...
* Quando si fa clic sul pulsante **[!UICONTROL Invio]** per un modulo nell’area di lavoro HTML, il modulo non viene chiuso ma ritorna nella propria coda di gruppo.
NPR-16352
...
* Il servizio di output non riesce a elaborare un PDF modificato in modo da includere un campo “milli-sec” aggiuntivo nei metadati `date-time format`. NPR-16838
...
* Sono state apportate correzioni inerenti all’utilizzo di tipi di pubblico per la sincronizzazione delle destinazioni, la pubblicazione di campagne e la selezione della metrica di obiettivo nell’interfaccia utente touch.
* Sono stati risolti alcuni problemi di sincronizzazione per i processi di traduzione.
* È stata potenziata la sicurezza del servizio di precompilazione di Forms.
* Sono stati apportati miglioramenti al componente per bozze e invii del portale Forms e nel servizio Barcoded Forms
...
* Nella schermata Sites dell’interfaccia touch, non è possibile incollare una pagina in un’altra pagina priva di pagine secondarie poiché non è disponibile il pulsante Incolla. NPR-15894: Hotfix per CQ-4201696.
* Durante lo scorrimento della scheda Pagine nel pannello Trova contenuto, alcuni gruppi di pagine vengono visualizzati indefinitamente nell’interfaccia classica, mentre nell’interfaccia touch viene mostrato un set limitato di poche pagine non ripetute. NPR-16271: Hotfix per CQ-4202371
...
* Quando nella gerarchia dei contenuti si crea una cartella con un titolo contenente caratteri speciali, viene visualizzata la versione codificata di tali caratteri. NPR-15935: Hotfix per CQ-4202987
...
* Quando si crea un’attività di test A/B di Target in AEM, il pubblico non si sincronizza con Target e viene visualizzato “Nessun pubblico”. NPR-16229: Hotfix per CQ-4204210
...
* Quando si utilizza il targeting, la pubblicazione di una campagna non comprende automaticamente l’intero albero, che include marchio ed elemento principale. NPR-15855: Hotfix per CQ-94630

### Traduzione {#translation-8}

* Il processo di sincronizzazione della traduzione ora viene attivato automaticamente e in il polling in AEM viene applicato senza interferire nei progetti di traduzione. NPR-15163: Hotfix per CQ-90856
...
* Durante il tentativo di salvare un frammento all’interno di un altro frammento nell’editor dei moduli, il valore del frammento secondario non viene recuperato dalla radice del modello di frammento. NPR-15943
...
* Per i moduli adattivi con widget file allegati, alcune parti del modulo vengono disattivate dopo l’anteprima dell’allegato.
NPR-16611

* Per i widget di allegati che consentono più allegati, l’invio di una nuova istanza di modulo con un allegato su un widget che ha già un allegato precedente genera la visualizzazione di un codice di errore. Questo errore si verifica all’apertura dell’allegato aggiunto anziché del contenuto effettivo. NPR-16258
* Proteggere il servizio di precompilazione dei moduli da accessi non autorizzati tramite protocolli quali `file://`, `http://` e `ftp://`. Consulta [Configurazione del servizio di precompilazione tramite Gestore di configurazione](https://experienceleague.adobe.com/it/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions). NPR-15414

* Richiesta di eseguire il rendering del modulo adattivo, nel passaggio di verifica, in formato PDF, anziché HTML, e di aggiungere al PDF tutti gli allegati, in modo che la stampa mostri il modulo completo. NPR-9011

**Gestione della corrispondenza**

* Quando si utilizza un frammento di testo in una lettera in modalità Anteprima o Modifica, se il testo viene convertito in elenco, l’intera funzionalità lettera si interrompe. Viene inoltre generato un errore di JavaScript. NPR-16460
...
* Se si lavora su un frammento di documento di testo nel browser Internet Explorer, non si riesce ad applicare un rientro al testo in quanto il cursore non consente tale operazione. NPR-16128
* L’aggiunta o la modifica di un dizionario dati a un frammento di documento di testo esistente richiede molto tempo e l’utente non sempre viene avvisato. NPR-16102
* Quando si visualizza in anteprima una lettera con contenuto scorrevole utilizzando Internet Explorer, la barra di scorrimento del browser si sovrappone alla barra di scorrimento della lettera. Di conseguenza, non è possibile visualizzare l’intero contenuto dei frammenti sul lato destro. NPR-16068
* Quando nel browser Google Chrome si creano o si modificano dei frammenti di documento di testo, il menu a discesa per la selezione del colore si apre automaticamente e non può essere rimosso. Per poter modificare il frammento, l’utente dovrà selezionare l’elenco come tipo di immissione dati. NPR-16067
...
* Durante il re-authoring il tipo di collegamento dati nelle lettere viene visualizzato come “Utente”, anche se se in precedenza era stato impostato diversamente. NPR-16619
...
#### Programma di installazione di Forms per JEE {#forms-jee-installer-17}

**Gestione dei processi - Area di lavoro HTML**

* Proteggere il servizio di precompilazione dei moduli da accessi non autorizzati tramite protocolli quali “file://”, “http://” e “ftp://”. Per informazioni dettagliate, consulta [Configurazione del servizio di precompilazione tramite il gestore di configurazione](https://experienceleague.adobe.com/it/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions). NPR-15434
...
* Il cambio delle impostazioni di lingua del dizionario in francese (Canada) non consente il funzionamento del controllo ortografico in AEM Forms Designer.
NPR-15896
...
Elenco di bundle OSGi aggiornati tra AEM 6.2 SP1 e CFP3
...
* Sono stati migliorati i flussi di lavoro di traduzione; è stato migliorato il connettore Microsoft® per l’utilizzo delle nuove API di traduzione per il portale Azure
...
* Una potenziale eccezione Null Pointer non viene impedita se si utilizza la chiamata alla funzione `com.adobe.granite.infocollector.impl.FilesTraversal`. NPR-15169: Hotfix per CQ-4197640
...
* Un utente seleziona un nodo in CRXDE (ad esempio, `/content/dam/`). Fa quindi clic sulla scheda Controllo degli accessi, verificando che esista un elenco di controllo degli accessi. Ora, con il trascinamento si spostano alcuni elementi, non quello selezionato. NPR-15696: Hotfix per GRANITE-16300
...
* Quando si crea uno schema metadati con numerosi campi e lo si applica ai metadati del frammento di contenuto, nella schermata dei metadati di quest’ultimo non viene creata alcuna barra di scorrimento, rendendo i campi non modificabili. NPR-15478: Hotfix per CQ-4202622
...
* Quando nell’interfaccia utente dell’editor di frammenti di contenuto vengono create molte varianti, nel pannello laterale non viene visualizzata la barra di scorrimento per poter accedere a tutte. NPR-15445: Hotfix per CQ-4199444
...
* Per un sito che utilizza Live Copy, quando si seleziona la casella di controllo “Live Copy” nel riquadro Colonne della console Site Admin, lo stato della Live Copy non viene visualizzato correttamente e viene visualizzato solo il markup HTML. NPR-15108: Hotfix per CQ-97086
...
* Se si chiude la pagina Modifica in modalità Timewarp e si tenta di riaprirla da Site Admin, viene generato un errore con stato `500`. NPR-14965: Hotfix per CQ-109647:
...
* Dopo aver cercato una risorsa in Omnisearch, selezionandola e provando a modificarne le proprietà facendo clic su **Visualizza proprietà** e quindi sul pulsante **Salva**, gli utenti verranno reindirizzati a una pagina vuota. NPR-15900: Hotfix per CQ-4202372
...
* Quando si pubblica una risorsa dalla vista a scheda, la scheda non viene aggiornata in modo da riflettere lo stato di pubblicazione. Al contrario, la pagina viene aggiornata. NPR-15826: Hotfix per CQ-102732
* Hotfix cumulativo contenente hotfix di Assets. NPR-15225
* Se il carattere “e commerciale” (&amp;) è incluso nel nome di una cartella risorsa, il nome della cartella non viene visualizzato correttamente quando si passa alla risorsa. NPR-15775: Hotfix per CQ-4201735
...
* Se si accede all’interfaccia “Le mie condivisioni di collegamenti” con il browser Firefox, non vengono visualizzati gli utenti o gli elementi condivisi e la schermata non è utilizzabile. NPR-15539: Hotfix per CQ-4200992
* Quando si utilizza Digital Asset Manager, se a una pagina è associato un set di immagini e si spostano le immagini in una nuova cartella, l’associazione di pagina non funziona. Di conseguenza, nella pagina associata mancano alcune immagini. NPR-15538: Hotfix per CQ-111479
...
* Durante la creazione di profili video, se vengono scelti predefiniti di codifica video di qualità sia media che alta, le modifiche apportate non vengono salvate. NPR-15447: Hotfix per CQ-4195482
...
* Se si utilizza la console Trova risorsa nella vista a colonne, il tentativo di creare una cartella non riesce la prima volta, ma funziona se si riprova. NPR-15370: Hotfix per CQ-4199448
* Se una risorsa o una cartella selezionata nell’interfaccia di DAM ha una virgola nel nome, la scheda Riferimenti non è disponibile e viene visualizzato un messaggio di tipo “Elenco di riferimenti non disponibile per selezioni multiple”. NPR-15362: Hotfix per CQ-4199721
...
* Dopo aver aggiornato AEM 6.2 a Service Pack 1, la sincronizzazione delle risorse in Scene7 ha esito negativo. La proprietà `dam:Scene7FileStatus` mostra lo stato `UploadFailed` anche per le risorse pubblicate. NPR-15269: Hotfix per CQ-4197708

### Interfaccia utente {#user-interface-5}

* Nell’**[!UICONTROL interfaccia Touch]**, la data salvata viene visualizzata per i campi data che ora presentano type=&#39;datetime&#39; se si utilizza il browser Internet Chrome versione 56.0.2924.87. NPR-15383: Hotfix per GRANITE-16481
...
* L’esecuzione di un flusso di lavoro Scene7 genera file aperti che non si chiudono. Richiesta di migliorare il servizio AEM-S7 in modo da mantenere e riutilizzare una singola istanza HttpClient con una configurazione di pooling condivisa. NPR-15357: Hotfix per CQ-109958

### Traduzione {#translation-9}

* Quando si utilizzano progetti di traduzione, l’aggiornamento delle copie in lingua dalla lingua principale inglese genera 11 avvii separati, tutti con lo stesso nome e radice di origine. Tuttavia, ciascuno di essi ha radici di lancio leggermente diverse, nel caso in cui il nome della pagina segua un pattern impostato. NPR-15605: Hotfix per CQ-4200699
...
* Se si utilizzano i componenti di integrazione Campaign, Targeting e `MAC`, Test e Target, l’annullamento della pubblicazione delle attività non aggiorna lo stato dell’attività nell’interfaccia principale. NPR-15401: Hotfix per CQ-4199839
...
* Il servizio di precompilazione non funziona per i moduli XDP. NPR-15466
* Quando si archiviano invii e bozze di moduli adattivi in un database, lo stato del modulo adattivo viene danneggiato se la connettività del database si interrompe per qualsiasi motivo (ad esempio, dopo un lungo periodo di inattività). NPR-15297

#### Programma di installazione di Forms per JEE {#forms-jee-installer-18}
...
* Se si utilizza il servizio di output per combinare un modulo specifico con un XML dati, il tempo di risposta risulta fino a 20 volte superiore rispetto al tempo impiegato dal server ES4 SP1. NPR-15283

#### App AEM Forms {#aem-forms-app-1}

* Durante il ripristino di attività non salvate, il messaggio mostrato deve essere reso più chiaro per poter ridurre gli errori degli utenti. NPR-15377
...
* Richiesta di aggiornare Jackrabbit File vault alla versione 3.1.30 per risolvere vari problemi. NPR-13454
...
* Il componente parsys WCM Sightly Foundation ignora i nomi dei tag di dichiarazione del componente: `cq:htmlTag / cq:tagName`. NPR-14225
...
* Gli elenchi a discesa di Target non funzionano nelle finestre di dialogo dell’interfaccia Touch se vengono creati più campi Rich Text, come i collegamenti. NPR-13911
* Quando in una finestra di dialogo (interfaccia utente touch) si modifica un campo di testo con più proprietà dell’editor Rich Text (RTE), una di queste viene attivata in modo casuale. NPR-13703
...
* Quando si invia la finestra di dialogo PageProperties dall’editor, la post-elaborazione di MSM scrive alcuni parametri dalla richiesta invece che il parametro da `msm:writeLiveCopyConfig`. NPR-14434
* Numerosi problemi relativi al componente Rollout, alle Live Copy e ad altri aspetti di MSM. NPR-12235

### Risorse {#assets-18}

* Il flusso di lavoro di decompressione non è in grado di gestire immagini con caratteri speciali nel nome del file. NPR-15227: Hotfix per CQ-103887
* Le risorse contenenti l’espressione “Repeat with condition” non vengono visualizzate correttamente. Quando l’utente visualizza l’anteprima del modello di lettera `*CDN3835RLCEN*`, non vengono visualizzate le risorse che si trovano nell’area di destinazione del corpo. Per i widget di allegati che consentono più allegati, l’invio di una nuova istanza del modulo con un allegato su un widget che ha già un allegato precedente determina la visualizzazione di un codice di errore. NPR-14844
...
* Gli utenti con diritti di replica ora possono eliminare cartelle di Assets. NPR-14321: Hotfix per CQ-88271
* Quando si tenta di modificare i profili di un video nell’editor canale, la finestra di dialogo di progettazione non si apre e nel registro degli errori viene generata un’eccezione Null Pointer. NPR-14144: Hotfix per CQ-81101
...
* La pagina Rapporti di Assets non risponde se l’utente seleziona tutte le opzioni disponibili nella finestra di dialogo `Configure Columns`. NPR-13187: Hotfix per CQ-95589
...
* I rapporti predefiniti di Assets generati in presenza di un elevato numero di risorse causano attraversamenti pesanti laddove la ricerca non coinvolge alcun indice, e quindi picchi di utilizzo della CPU. NPR-12811: Hotfix per CQ-84409
...
* Dall’aggiornamento di `Jquery`, gli utenti non riescono a selezionare una raccolta di risorse e a confermarne la selezione nel pannello Contenuto associato di un frammento di contenuto. NPR-14847: Backport per CQ-4194209
...
* Il cookie saml_request_path configurato utilizzando lo strumento di gestione delle autorizzazioni SAML mostra il cookie impostato con un carattere aggiuntivo `?`. Se viene eseguito il postback di una risposta SAML in AEM, il cookie AEM saml_request_path restituisce un valore non valido a causa di caratteri già codificati. NPR-13517: Hotfix proattivo per GRANITE-11722 e GRANITE-14414

### Dynamic Media {#dynamic-media}

* Numerosi processi Sling `AEM-Scene7` creati e annullati durante l’attivazione di AEM vengono registrati come processi di archiviazione durante la replica. NPR-12835: Hotfix per CQ-86115
...
* Richiesta proattiva per la correzione del comportamento XSS (Cross-Site Scripting) durante la procedura guidata per la creazione di lanci.
...
* Se si imposta un valore predefinito per un pulsante di scelta e si seleziona un valore diverso durante il salvataggio o l’invio, il valore selezionato non viene visualizzato nella precompilazione. NPR-15304
...
* Quando si utilizza la modalità di modifica per i moduli adattivi, si verificano errori JavaScript di tipo “Gestore del componente non valido” e “guidelib non definita”. NPR-15112
...
* Il pannello Elementi dati nell’utility Generatore di espressione continua a caricarsi nel caso in cui viene creato un modulo di condizione, senza aver selezionato un dizionario dei dati. NPR-14688
...
* L’evento di audit relativo all’accesso utente viene registrato con un’ora non valida. L’ora corretta per l’evento di audit non è tracciabile. NPR-13107
...
* Forms Designer si chiude inaspettatamente se l’utente esegue l’utlity di controllo ortografico con la lingua Francese (Canada) selezionata nelle impostazioni internazionali modulo. NPR-13740
...
>[!NOTE]
>
>Per i clienti di AEM Forms, è essenziale installare il pacchetto dei componenti aggiuntivi dopo l’installazione di qualunque Service Pack, Cumulative Service Pack o Feature Pack di AEM.

Puoi scaricare il pacchetto CFP direttamente da Software Distribution oppure eseguire i seguenti passaggi:
...
>[!NOTE]
>
>I Feature Pack opzionali forniti da Adobe dipendono dalla versione di rilascio e dal Cumulative Fix Pack. Se hai installato un Feature Pack, contatta il [team di Assistenza clienti di AEM](https://experienceleague.adobe.com/i?support-solution=General#support) per verificare la compatibilità con questo Cumulative Fix Pack per AEM 6.2.
...
* AEM 6.2 Service Pack 1 è un prerequisito per CFP. Per le istruzioni di installazione, consulta le note sulla versione di [AEM 6.2 Service Pack 1](https://experienceleague.adobe.com/it/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
* Per una distribuzione in cluster tramite RDBMK o MongoDB, il pacchetto CFP può essere installato in qualsiasi istanza di authoring che utilizza Gestione pacchetti.

* Prima di installare il Cumulative Fix Pack, conviene creare uno snapshot o eseguire un backup dell’istanza AEM.
...
1. Per scaricare il pacchetto, fai clic su [Software Distribution](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq640/cumulativefixpack/aem-6.4.8-cfp-2.0.zip).
...
* Utilizza l’[API HTTP da Gestione pacchetti](https://experienceleague.adobe.com/it/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions). In questo caso assicurati di utilizzare `cmd=install&recursive=true` in modo che il pacchetto nidificato venga installato.

### Convalidare l’installazione {#validate-installation}

1. Nella pagina Informazioni prodotto (/system/console/ productinfo), in Prodotti installati ora deve essere visualizzata la stringa della versione aggiornata “Adobe Experience Manager, versione 6.2.0.SP1-CFP20”.
...
1. Installa il pacchetto dei componenti aggiuntivi per Forms come descritto in [Installazione dei pacchetti aggiuntivi per AEM Forms](https://experienceleague.adobe.com/it/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions).
...
## Parametri di timeout configurabili dall’utente per le connessioni a DTM, Analytics e Target {#user-configurable-timeout-parameters-for-dtm-analytics-target-search-promote-connections}
...
| **Connessioni** | **Timeout della connessione&#42;** | **Timeout del socket&#42;&#42;** |
|---|---|---|
| DTM | 30000 millisecondi | 30000 millisecondi |
| Analisi | 30000 millisecondi | 30000 millisecondi |
| Destinazione | 60000 millisecondi | 30000 millisecondi |

* **Timeout della connessione&#42;**: timeout in millisecondi fino al momento in cui viene stabilita una connessione. Un valore di timeout pari a zero viene interpretato come un timeout infinito.
* **Timeout del socket&#42;&#42;**: timeout in millisecondi in attesa dei dati o per un periodo massimo di inattività tra due pacchetti consecutivi di dati.
...
## L’aggiornamento Java™ 8 131 più recente presenta un’eccezione (NPR-21355) {#latest-java-update-throws-an-exception-npr}

>[!NOTE]
>
>Queste impostazioni di configurazione sono specifiche per la clientela di AEM Forms che utilizza il servizio Document Security.

NPR-21355 è incluso in CFP12.1. Se stai installando CFP 12.1 o una versione successiva, segui la procedura allegata per configurare NPR-21355 sul server applicazioni JBoss®. Se stai installando CFP12.1 sul server AEM Forms in esecuzione sui server applicazioni Oracle WebLogic o IBM® WebSphere, non è richiesta alcuna configurazione aggiuntiva:

1. Esegui il backup, elimina e crea un file module.xml. Il percorso predefinito del file è [AEM_Forms_Installation_directory]/jboss/module/system/layer/base/com/adobe/livecycle/main/
...
1. Crea un backup dei file `jsafeFIPS.jar`, `jsafeJCEFIPS.jar`, e `certjFIPS.jar` in [AEM_Forms_Installation_directory]`/jboss/modules/system/layers/base/com/adobe/livecycle/main/` ed elimina i file dalla directory indicata in precedenza.
...
* In caso di server JBoss® in una configurazione indipendente, apri il file standalone.conf.bat per la modifica.
* In caso di server JBoss® in configurazione cluster, apri il file domain.conf.bat per la modifica.

  Aggiungi le seguenti righe alla fine e salva il file:

  Imposta `JAVA_OPTS=%JAVA_OPTS%-Djnlp.com.rsa.cryptoj.fips140loader=true`

  Imposta `JAVA_OPTS=%JAVA_OPTS%-Dcom.rsa.cryptoj.fips140initialmode=NON_FIPS140_MODE`
...
* In caso di server JBoss® in una configurazione indipendente, apri il file standalone.conf per la modifica.
* In caso di server JBoss® in una configurazione cluster, apri il file domain.conf per la modifica.
...
Quando un utente richiede un’attività, il conteggio della coda condivisa non viene aggiornato per gli altri utenti. Di conseguenza, Adobe ha introdotto una nuova proprietà. Per configurare questa proprietà in un’istanza di AEM, segui la procedura seguente:
...
1. Aggiungi il tag `<client_tasksPollingInterval>10</client_tasksPollingInterval>` nel file XML scaricato. Dieci è il valore di esempio espresso in secondi. È possibile modificarlo in base alle proprie esigenze.
1. Salva il file.
...
* A causa del riavvio dell’archivio si verificano errori quali **create component instance** e **Service factory returned null**:
...
* (Adobe Granite Maintenance Scheduler Update Task) com.adobe.granite.maintenance.impl.TaskScheduler: nessuna attività di manutenzione trovata con nome WorkflowPurgeTask per la finestra `granite:weekly`
...
* Quando si installa CFPx su AEM 6.2 SP1 includendo il feature pack Tag avanzati, il passaggio del flusso di lavoro aggiunto precedentemente per le risorse di tag avanzati viene eliminato dal flusso di lavoro Aggiorna risorsa DAM.

## JAR Uber {#uber-jar}

Il file JAR Uber per 6.2 SP1-CFP20 è disponibile nell’archivio pubblico Maven di Adobe.

Per utilizzare il file JAR Uber in un progetto Maven, includi nel POM del progetto la seguente dipendenza:
...
>[!MORELIKETHIS]
>
>* [Pagina degli hotfix di AEM 6.2](https://experienceleague.adobe.com/it/docs/experience-manager-release-information/aem-release-updates/aem-releases-updates)
>* [Note sulla versione di AEM 6.2 SP1](https://experienceleague.adobe.com/it/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [Note sulla versione di AEM 6.2](https://experienceleague.adobe.com/it/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [Pagina del prodotto AEM](https://business.adobe.com/it/products/experience-manager/adobe-experience-manager.html)
>* [Documentazione di AEM 6.2](https://experienceleague.adobe.com/it/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions)
>* [Adobe Priority Product Updates](https://experienceleague.adobe.com/it/docs/release-notes/experience-cloud/current)
